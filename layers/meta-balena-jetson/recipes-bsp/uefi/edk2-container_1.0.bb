...
do_compile () {
    mkdir -p ${UNPACKDIR}/out
    cp ${UNPACKDIR}/Dockerfile ${B}/
    cp ${UNPACKDIR}/build.sh ${B}/
    chmod +x ${B}/build.sh
    cp -r ${UNPACKDIR}/*_patch.txt ${B}/
    ...
    DOCKER_API_VERSION=1.24 docker run --rm -v ${UNPACKDIR}/out:/out -v "${HOME}":"${HOME}" -e EDK2_DOCKER_USER_HOME="${HOME}" -e DEVICE_TYPE="${MACHINE}" -e "DEFAULT_DTB=${DEFAULT_DTB}" ${IMAGETAG} su /bin/bash -c "/build/build.sh && cp -r /build/nvidia-uefi/Build/t23x_general/DEBUG_GCC/AARCH64/L4TLauncher.efi /out/BOOTAA64.efi && cp /build/nvidia-uefi/images/uefi_t23x_general_DEBUG.bin /out/uefi_jetson.bin"
...
do_install() {
    ...
    install -m 0644 ${UNPACKDIR}/edk2_License.txt ${D}/opt/tegra-binaries/edk2/License.txt
    install -m 0644 ${UNPACKDIR}/edk2-platforms_License.txt ${D}/opt/tegra-binaries/edk2-platforms/License.txt
    install -m 0644 ${UNPACKDIR}/edk2-nvidia_LICENSE ${D}/opt/tegra-binaries/edk2-nvidia/LICENSE
    install -m 0644 ${UNPACKDIR}/NOTICE.nvgop-chips-platform.efi ${D}/opt/tegra-binaries/edk2-nvidia-non-osi/
}

do_deploy () {
    rm -rf ${DEPLOY_DIR_IMAGE}/bootfiles/EFI/BOOT || true
    mkdir -p ${DEPLOY_DIR_IMAGE}/bootfiles/EFI/BOOT/
    cp ${UNPACKDIR}/out/uefi_jetson.bin ${DEPLOY_DIR_IMAGE}/
    cp ${UNPACKDIR}/out/BOOTAA64.efi ${DEPLOY_DIR_IMAGE}/bootfiles/EFI/BOOT/
}
