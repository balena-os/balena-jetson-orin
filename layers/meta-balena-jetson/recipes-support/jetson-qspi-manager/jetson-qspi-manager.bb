...
RDEPENDS:${PN} += " bash setup-nv-boot-control "

inherit allarch systemd l4t_version
...
do_install() {
    ...
    install -m 0755 ${UNPACKDIR}/jetson-qspi-manager ${D}${bindir}/
    install -m 0644 ${UNPACKDIR}/jetson-qspi-helpers ${D}/${libexecdir}/

    sed -i -e 's,@ORIN_DEVICE_TYPE@,${COMPAT_SPEC_NAME},g' ${D}/${libexecdir}/jetson-qspi-helpers
    sed -i -e 's,@L4T_VER@,${L4T_VERSION},g' ${D}/${libexecdir}/jetson-qspi-helpers

    if ${@bb.utils.contains('DISTRO_FEATURES','systemd','true','false',d)}; then
        install -d ${D}${systemd_unitdir}/system/
        install -d ${D}${sysconfdir}/systemd/system/multi-user.target.wants/
        install -m 0644 ${UNPACKDIR}/jetson-qspi-manager.service ${D}${systemd_unitdir}/system/
